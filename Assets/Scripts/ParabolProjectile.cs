...
public class ParabolProjectile : Projectile
{
			...
			if (!this.isInCollision)
			{
				...
				for (int i = 0; i < array2.Length; i++)
				{
					...
					if (component)
					{
						...
						if (component == tt)
						{
							SoundController.instance.PlaySoundItem1();
							component.CallFlash((double)(BaseValue.vertical_slam_base_damage * (long)coefLevel_), BaseValue.coin_per_item1_hit, ProjectileType.Non_Projectile);
						}
						else
						{
							component.CallFlash((double)((long)((float)(BaseValue.vertical_slam_base_damage * (long)coefLevel_) / (100f / (float)BaseValue.damage_percent_item * (float)num))), (long)((float)BaseValue.coin_per_item1_hit / (100f / (float)BaseValue.damage_percent_item * (float)num)), ProjectileType.Non_Projectile);
						}
					}
				}
				GameObject pooledObject = ParticleObjectPooler.instance.GetPooledObject("item1_particle");
			...
			if (!this.isInCollision)
			{
				...
				for (int j = 0; j < array4.Length; j++)
				{
					...
					if (component2)
					{
						...
						component2.CallFlash((double)((long)((float)(BaseValue.vertical_slam_base_damage * (long)coefLevel_2) / (100f / (float)BaseValue.damage_percent_item * (float)num2))), (long)((float)BaseValue.coin_per_item1_hit / (100f / (float)BaseValue.damage_percent_item * (float)num2)), ProjectileType.Non_Projectile);
					}
				}
				GameObject pooledObject2 = ParticleObjectPooler.instance.GetPooledObject("item1_particle");
