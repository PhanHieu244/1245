...
public class SmartMissile3D : SmartMissile<Rigidbody, Vector3>
{
	...
	{
		Collider[] array = Physics.OverlapSphere(base.transform.position, this.m_searchRange);
	...
	protected override void goToTarget()
	{
		...
		this.m_rigidbody.velocity = Vector3.ClampMagnitude(this.m_rigidbody.velocity + this.m_direction * this.m_guidanceIntensity, this.m_rigidbody.velocity.magnitude);
		if (this.m_rigidbody.velocity != Vector3.zero)
		{
			base.transform.LookAt(this.m_rigidbody.velocity);
		}
	}
}
